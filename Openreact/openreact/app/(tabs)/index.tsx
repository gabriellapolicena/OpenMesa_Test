...
export default function App() {
  ...
  // Pedidos de Convênio
  const [pedidosConvenio, setPedidosConvenio] = useState<Pedido[]>([]);
  const [pedidoSelecionadoDetalhe, setPedidoSelecionadoDetalhe] = useState<Pedido | null>(null);
  const [pedidosSelecionadosPagamento, setPedidosSelecionadosPagamento] = useState<string[]>([]);
  ...
  useEffect(() => {
    const executarBackupAutomatico = async () => {
      try {
        ...
        const dadosParaBackup: Record<string, any> = {};
  ...
  const temPermissao = (chavePermissao: string): boolean => {
  ...
  const realizarCadastroTelaLogin = async () => {
    ...
    const novoUsuarioObj = {
      ...
      permissoes: [...(PERMISSOES_PADRAO[perfilAtribuido as keyof typeof PERMISSOES_PADRAO] || [])]
    };
  ...
  const obterChaveData = (data: Date): string => {
  ...
  const buscarDadosData = async (data: Date) => {
    const chave = obterChaveData(data);
    const dados = await AsyncStorage.getItem(chave);
    return dados ? JSON.parse(dados) : { refeicao: 0, marmita: 0, convenio: 0, pendentes: 0 };
  };

  const salvarDadosData = async (data: Date, r: number, m: number, c: number = 0, pendentesAdicionais: number = 0) => {
    const chave = obterChaveData(data);
  ...
  const somarDadosDiaAtual = async (addRefeicao: number, addMarmita: number, addConvenio: number = 0) => {
    const hoje = new Date();
  ...
  const salvarEdicaoPedidoConvenio = async (novoQtd: string | number) => {
    if (!pedidoSelecionadoDetalhe) return;
    const qtd = parseInt(String(novoQtd)) || 0;
  ...
  const alternarSelecaoPagamento = (id: string) => {
    ...
    try {
      ...
      for (const key of keysData) {
        ...
        if (tipo === 'Diário') {
          ...
          incluir = (dtInicio && dtFim) ? (dataItem >= dtInicio && dataItem <= dtFim) : false;
        }
      ...
      dadosFiltrados.sort((a: any, b: any) => new Date(a.dataObj).getTime() - new Date(b.dataObj).getTime());
  ...
  const criarNovoUsuario = async () => {
    ...
    const novo = {
      ...
      permissoes: [...(PERMISSOES_PADRAO[novoUserPerfil as keyof typeof PERMISSOES_PADRAO] || [])]
    };
  ...
  const alternarPermissaoUsuario = async (userId: string, permissao: string) => {
  ...
  const abrirRegistroData = async (dataSelecionada: Date) => {
  ...
  const navegarDataRegistro = async (dias: number) => {
    const novaData = new Date(dataRegistro);
    novaData.setDate(novaData.getDate() + dias);
    await abrirRegistroData(novaData);
  };

  const alterarQtdRegistro = async (tipo: string, valor: string) => {
  ...
  const ehHoje = (data: Date): boolean => {
    const hoje = new Date();
      ...
      <Modal visible={modalAtivo === 'pop_confirmacao'} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <TouchableOpacity style={styles.btnPrincipal} onPress={avancarParaSucesso}>
            ...
            <ScrollView style={{ maxHeight: 350, width: '100%' }}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 }}>
                {['CAIXA', 'FISCAL', 'AUDITOR', 'EMPRESA'].map(p => (
                  <TouchableOpacity 
                    key={p} 
                    style={[styles.btnFechar, novoUserPerfil === p && { backgroundColor: '#1a73e8' }]} 
              ...
              {usuarios.map(u => (
                <View key={u.id} style={[styles.itemPedido, { flexDirection: 'column', alignItems: 'flex-start' }]}>
                  ...
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 4 }}>
                    {Object.keys(PERMISSOES_PADRAO).map(key => PERMISSOES_PADRAO[key as keyof typeof PERMISSOES_PADRAO]).flat().reduce((acc: string[], current: string) => {
